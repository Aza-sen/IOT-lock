...
void setup()
{
  ...
  pinMode(D4, OUTPUT); // D4 to D7 are led stripe
  pinMode(D5, OUTPUT);
  pinMode(D6, OUTPUT);
  pinMode(D7, OUTPUT);
  pinMode(D8, INPUT); // magnetic sensor
...
void loop()
{
  ...
  if(digitalRead(D8)==HIGH){   // if sensor is high

  // then switch off the lock and led and virtual write the resp lock
    delay(3000);
    digitalWrite(D4, HIGH);
    digitalWrite(L1, HIGH);
